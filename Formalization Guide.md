...
- `On <Target> Action [[<Ability Type>]]` for triggering when a certain target type uses a action with any (`On <> Action`) or a specific ability (e.g. `On <> Action [Investigation]`) (Use `@ActionTarget` to select the player the action is being used on, `@ActionResult` for the action's result, `@This` to reference the visited player).
