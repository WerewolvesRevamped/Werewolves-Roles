...
- `On [Active|Passive|Partial|Recruitment] Defense` for an ability that triggers when an active, passive, partial or recruitment defense is used (Use `@Attacker` within this trigger to reference the player (if existing) responsible for the defense being used, use `@AttackSource` to get the source of the attack)
...
- `@Selection[<Type>]`: Specifies a selection must be from a specific type. Can be `Player`, `Dead`, `Role`, `Category`, `Full Category`, `Attribute` or `List: <List Elements>`.
